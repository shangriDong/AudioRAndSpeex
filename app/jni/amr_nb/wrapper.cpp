...
int Encoder_Interface_Encode(void* s, int mode, const short* in, unsigned char* out) {
	struct encoder_state* state = (struct encoder_state*) s;
	enum Frame_Type_3GPP frame_type = (enum Frame_Type_3GPP) (enum Mode)mode;
	int ret = AMREncode(state->encCtx, state->pidSyncCtx, (enum Mode)mode, (Word16*) in, out, &frame_type, AMR_TX_IETF);
	out[0] |= 0x04;
	return ret;
}
#endif
